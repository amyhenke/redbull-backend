...
export const findAll = async () => {
    try {
        ...
        throw new Error("Failed to fetch cards data from DB");
    }
};
...
//     try {
//         const data = await sql`SELECT * FROM cards WHERE id = ${id}`;
//         console.log(data);
...
export const findBySlug = async (slug) => {
    try {
        const data = await sql`SELECT * FROM cards WHERE slug = ${slug}`;
        console.log(data);
        if (data.rows.length === 0) {
